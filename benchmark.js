...
retext = new Retext();
retextWithSyllable = new Retext().use(syllable);
...
suite('retext w/o retext-syllable', function () {
...
suite('retext w/ retext-syllable', function () {
    bench('A paragraph (5 sentences, 100 words)', function (done) {
        retextWithSyllable.parse(paragraph, done);
    });

    bench('A section (10 paragraphs, 50 sentences, 1,000 words)',
        function (done) {
            retextWithSyllable.parse(section, done);
        }
    );
});
